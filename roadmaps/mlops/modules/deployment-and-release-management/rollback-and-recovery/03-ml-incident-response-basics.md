...
For ML services, alert on a small set of symptoms:

- Request success, latency, and saturation for the serving path.
- Prediction volume, empty response rate, and fallback rate.
- Business or product proxies, such as ETA absolute error once labels arrive.
- Data quality failures, such as missing required features or impossible feature values.
- Drift or segment anomalies that have historically led to bad outcomes.
- Release health, such as canary error rate, rollback failure, or version mismatch.
...
For a prediction-quality incident, your runbook can start with four questions:

- Did the model version change?
- Did the input data change?
- Did traffic composition change?
- Did a dependency or fallback path change?

Then give responders exact commands and queries:
...
For `routewise-eta`, the review might find:

- The canary passed global mean absolute error, yet west-london rainy deliveries were too small a slice.
- The rain feature freshness metric existed, yet it had no alert.
- The model alias rollback worked, yet the service cached the model for 20 minutes.
- Support tickets detected user pain faster than monitoring.

Turn those into durable fixes:
...
## Practical Checks
<!-- section-summary: You are ready for ML incident response when each production model has:. -->

You are ready for ML incident response when each production model has:

- A named owner and escalation path.
- A severity guide tied to user impact.
- A dashboard that shows service health, model version, input quality, prediction distribution, and business proxy metrics.
- A rollback path tested outside a real incident.
- A runbook with exact commands and queries.
- An incident template that captures timeline, impact, mitigation, root cause, and follow-ups.
- A release process that links model versions to evaluation evidence.

During interviews, a strong answer sounds practical: "I would first confirm user impact and current model version, then check recent deploys and feature freshness, then choose rollback or fallback while keeping a clear timeline." That answer shows you understand both ML behavior and production operations.
