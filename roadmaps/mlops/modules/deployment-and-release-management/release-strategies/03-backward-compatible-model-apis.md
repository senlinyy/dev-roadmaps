...
overview: "Backward-compatible model APIs keep approved requests and useful responses stable while the model version changes behind the endpoint. This guide follows a delivery ETA model through request and response contracts, MLflow and Databricks aliases, rollout gates, compatibility tests, and owners."
...
1. [Why Model API Compatibility Matters](#why-model-api-compatibility-matters)
2. [The Compatibility Map](#the-compatibility-map)
3. [The Serving Contract](#the-serving-contract)
4. [Adding Fields Without Breaking Callers](#adding-fields-without-breaking-callers)
5. [Model Aliases and Version References](#model-aliases-and-version-references)
6. [Compatibility Tests in CI](#compatibility-tests-in-ci)
7. [Canary Gates and Owners](#canary-gates-and-owners)
...
If the team changes the endpoint carelessly, small serving details can create product outages. A field called `delivery_minutes` might change from an integer to a string. A new required request field might make old mobile apps fail. A confidence score might move from `confidence` to `prediction.confidence`, and the dispatcher screen might silently hide important warnings. The model might have higher offline accuracy while the product breaks at the API boundary.
...
| **Response contract** | What callers can expect back | `eta_minutes`, `confidence`, `model_version`, and `explanation_codes` stay available |
...
The response schema allows extra fields with `additionalProperties: true`. That choice gives the model API room to add optional metadata later. For example, version 18 can add `calibration_band` while old clients keep reading `eta_minutes` and `confidence`. Some client SDKs still use strict generated types, so the API owner should also tell client owners to ignore unknown response fields during deserialization.
...
This is where model APIs differ from ordinary CRUD APIs. A field can keep the same name and type while the meaning drifts. If `confidence` used to mean calibrated probability and version 18 changes it to a ranking score, the JSON shape stayed the same while the product contract changed. The dispatcher screen might hide risky estimates because it still treats `0.92` as a probability.
...
x-ml-contract:
  output_meaning:
    ...
    confidence: "Calibrated confidence score from 0 to 1 for this ETA band."
  ...
  contract-test:
    ...
    steps:
      ...
      - uses: actions/setup-python@v6
        with:
          python-version: "3.12"
      - run: pip install -r services/eta-api/requirements-dev.txt
...
This alert catches a classic compatibility failure: old callers send requests that the candidate endpoint rejects. The team should also watch `5xx` errors, p95 latency, fallback rate, missing feature rate, and online error proxy metrics such as customer ETA correction events. When labels arrive later, the model owner should compare actual arrival times against predictions by city, vehicle type, weather band, and app version.
