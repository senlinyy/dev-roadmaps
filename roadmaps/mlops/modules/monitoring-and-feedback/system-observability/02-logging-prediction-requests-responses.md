...
overview: "Prediction logging records the safest useful facts around each inference call. This tutorial follows an image moderation model service through structured JSON logs, request IDs, payload boundaries, redaction rules, OpenTelemetry log correlation, sampling, retention policies, and incident triage."
...
1. [Prediction Logs Preserve The Story Around A Decision](#prediction-logs-preserve-the-story-around-a-decision)
2. [Follow One Image Moderation Service](#follow-one-image-moderation-service)
3. [Decide The Safe Logging Boundary](#decide-the-safe-logging-boundary)
4. [Attach Request IDs And Structured Fields](#attach-request-ids-and-structured-fields)
5. [Log The Response Without Leaking The Payload](#log-the-response-without-leaking-the-payload)
6. [Redact, Sample, And Retain Logs Deliberately](#redact-sample-and-retain-logs-deliberately)
7. [Use Logs During Incidents And Reviews](#use-logs-during-incidents-and-reviews)
...
## Follow One Image Moderation Service
<!-- section-summary: The running scenario is an image moderation API where the team needs debugging evidence while keeping raw images and private user details out of general logs. -->
...
Here is a FastAPI middleware pattern that accepts an incoming request id or creates one. It stores the id on `request.state`, returns it to the caller, and leaves the endpoint free to write prediction events with the same id.
...
The endpoint code can write this event after inference:
...
Here is a collector fragment that deletes tokenized URLs, hashes a user id field if an older service still sends it, and keeps request ids and trace ids for correlation:
...
An Elastic Index Lifecycle Management policy can express time-based log retention:
...
The policy handles the general prediction-log index. The archived audit copy lives in a separate controlled store with its own retention and access rules. Separating those paths keeps daily debugging fast while keeping sensitive review evidence under a stricter workflow.
...
The responder filters logs for the model version, the decision, and the source surface:
...
In an interview, you can describe prediction logging like this: prediction logs are structured records for inference events. They connect a request id, trace id, model version, sanitized input summary, prediction output, policy decision, latency, and status. The key engineering skill is choosing a safe boundary. Metrics show aggregate health, traces show the path through services, and prediction logs explain the specific decision in a way teams can query, review, redact, sample, and retain deliberately.
