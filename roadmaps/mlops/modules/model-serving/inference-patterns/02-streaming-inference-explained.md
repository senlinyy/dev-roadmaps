...
overview: "Streaming inference scores events as they flow through a stream instead of waiting for a batch window or a live HTTP request. This guide follows a mobility team that consumes trip events from Kafka, scores them for safety risk, and operates the consumer with lag checks, dead-letter handling, and KEDA autoscaling."
...
Here is a validation report the worker can write every five minutes:
...
KEDA can scale Kubernetes consumers from Kafka lag. KEDA watches the event source and feeds metrics to Kubernetes HPA, so the deployment can scale out while work is pending and scale back when the stream quiets down. The Kafka scaler also documents an important partition reality: replicas normally should not exceed partition count, because extra consumers can sit idle when there are more consumers than partitions.
