...
overview: "After choosing a serving pattern, teams need to operate its data contracts, runtime, outputs, recovery, and model identity. This article explains batch and online paths and when one product needs both."
...
The previous article chooses serving patterns from product requirements. **Batch inference** runs a model over prepared records and writes predictions for later use. **Online inference** answers during a live request. This article owns how those two paths operate and how they can share one model contract without sharing the same failure behaviour.
...
:::expand[Implement the batch scoring job]{kind="example"}
    ...
    spec:
      ...
      template:
        spec:
          ...
          containers:
            - name: scorer
              image: registry.example.com/ml/substitution-risk@sha256:a3b9fb66e2716a43ff0ab8172369e0a101659f96f1681fbf727ab4c596047924
              args:
...
:::expand[Implement the online prediction endpoint]{kind="example"}
...
Online inference checks start with live service behavior. The team watches request rate, 95th-percentile (**p95**) and 99th-percentile (**p99**) latency, error rate, timeout count, fallback rate, CPU or GPU saturation, and the rate of validation failures. P95 is the response time that 95 percent of requests meet or beat; p99 covers 99 percent and exposes a slower tail. These signals tell the team whether the customer workflow is healthy and whether the model service has enough capacity.
