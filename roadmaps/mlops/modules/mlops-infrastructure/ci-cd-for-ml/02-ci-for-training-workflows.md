...
<!-- section-summary: Training CI is the set of automated checks that runs before a training change is merged or promoted. It is the safety net between a developer's pull request and a pipeline that... -->
...
For GPU training, CI may run only a CPU smoke test, while a scheduled GPU validation job checks CUDA, NCCL, and accelerator availability. The key is to make that split explicit. The PR check proves the image shape; the scheduled job proves the expensive runtime.
...
jobs:
  fast-training-checks:
    ...
    steps:
      ...
      - run: python -m compileall claimlens
      - run: pytest tests/unit tests/contracts tests/smoke -q

  integration-training-check:
    if: contains(github.event.pull_request.labels.*.name, 'run-training-integration')
    runs-on: ubuntu-latest
    environment: ml-dev
    permissions:
      id-token: write
    ...
    steps:
      ...
      - uses: actions/setup-python@v6
        with:
          python-version: "3.12"
      - run: pip install -r requirements-dev.txt
      - run: pytest tests/integration/test_training_data_access.py -q
      - run: python -m claimlens.train --config configs/claim_severity_dev.yml --max-rounds 20
```

The `environment` setting is useful because many teams require approval before jobs with cloud credentials run. OpenID Connect federation can avoid long-lived cloud secrets in CI.
...
<!-- section-summary: A good setup gives developers fast confidence and gives platform teams guardrails. When a PR breaks a feature transform, the unit test fails. When a config references a missing... -->
...
## A Practical Training CI Checklist
<!-- section-summary: Before you call a training workflow production-ready, walk through this checklist:. -->

Before you call a training workflow production-ready, walk through this checklist:
...
If half of these are missing, start with the cheapest three: config parsing, smoke training, and metadata checks. Those three catch a surprising amount of breakage before the expensive run starts.
