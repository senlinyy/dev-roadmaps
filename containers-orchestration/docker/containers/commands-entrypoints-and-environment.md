...
This is where many Docker mistakes begin. A developer thinks they changed the image, but they only overrode the command for one container. Another developer passes a new argument and accidentally appends it to an entrypoint. Someone bakes a staging database URL into the image because it made a local run pass. The container starts, but the source of truth is now blurred.
...
passes `-version` to the `terraform` entrypoint. The image behaves like the binary. That is a good fit for tool images.
