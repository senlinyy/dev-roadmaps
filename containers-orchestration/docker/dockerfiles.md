---
title: "Dockerfiles"
description: "Write Dockerfiles that package an application with the right files, runtime, command, and operational defaults."
overview: "A Dockerfile is the recipe that turns source code into a runnable image. Good Dockerfiles make runtime behavior explicit, keep build tools out of production images, and leave useful evidence when something fails."
tags: ["dockerfile", "images", "build", "healthcheck"]
order: 2
...
1. [Write the Runtime Contract](#write-the-runtime-contract)
...
10. [Practice Loop for Image Review](#practice-loop-for-image-review)

## Write the Runtime Contract

A Dockerfile answers one practical question: what should exist around your application
when it starts? Your repository has code, package metadata, tests, editor files, and
local notes. A production container needs a smaller set: a runtime, installed
dependencies, application files, default environment, a working directory, and the
command that becomes PID 1 inside the container.
...
The key mental model for this article is `Dockerfile`. Do not treat it as vocabulary to
...
## Practice Loop for Image Review

Before a Dockerfile becomes normal team infrastructure, review it by
building, running, and inspecting the image. The goal is not to memorize
every instruction. The goal is to prove that the image contains the
files it claims to contain, runs the command it claims to run, and
avoids carrying local-only state.

```bash
$ docker build -t devpolaris/orders-api:review .
$ docker run --rm --name orders-api-review   -p 127.0.0.1:8080:3000   -e DATABASE_URL=postgres://orders:orders@host.docker.internal:5432/orders_dev   devpolaris/orders-api:review
```

In another terminal, prove the service path before you inspect
internals.
...
Then inspect the image metadata. This catches a surprising number of
review mistakes: old commands, wrong working directories, missing users,
and environment defaults that were copied from a laptop experiment.

```bash
$ docker image inspect devpolaris/orders-api:review   --format "workdir={{.Config.WorkingDir}} user={{.Config.User}} cmd={{.Config.Cmd}}"
workdir=/app user=node cmd=[node dist/server.js]
```

If `user` is empty, the image probably runs as the default user from the
base image. If `cmd` points to `server.js` while the Dockerfile copies
`dist`, the image contract and build output disagree. If the working
directory is empty, relative paths in the command deserve another look.

Finally, check that sensitive local files did not enter the image. This
is a quick local check, not a replacement for image scanning, but it
catches the common beginner mistake.

```bash
$ docker run --rm devpolaris/orders-api:review sh -c 'find /app -maxdepth 2 -name ".env*" -o -name "npm-debug.log"'
```

No output is the expected result. If `.env` appears, fix `.dockerignore`
and rebuild. If `npm-debug.log` appears, remove local noise from the
build context. The review loop keeps the Dockerfile honest: build the
image, run the service, inspect metadata, and look for files that should
never have crossed into the image.

One more review check is to compare the image tag with the container that is actually running. A common local mistake is rebuilding `devpolaris/orders-api:review` and then testing an older container that still points at a previous image ID.

```bash
$ docker ps --filter name=orders-api-review --format "{{.Image}} {{.ID}}"
devpolaris/orders-api:review a18d7b44cb20

$ docker inspect orders-api-review --format "{{.Image}}"
sha256:4d817d31c0a8f3a89d81d4c3d8a4e0f40f7b8b7ec45261a4b80a57b35c1e6d22
```

If the image was rebuilt after the container started, recreate the container before trusting the test result.

If the review command and the health check disagree, trust the evidence nearest to the running process. The Dockerfile describes intent. The container metadata and logs show what Docker actually started.

---

**References**

- [Docker Docs: Dockerfile reference](https://docs.docker.com/reference/builder) - Canonical reference for Dockerfile instructions and their behavior.
- [Docker Docs: Building best practices](https://docs.docker.com/build/building/best-practices/) - Official guidance for creating maintainable images.
- [Docker Docs: Multi-stage builds](https://docs.docker.com/build/building/multi-stage/) - Explains how to separate build stages from runtime stages.
