...
If the process runs as one uid and the mounted directory is owned by another, the failure may look like an application error even though the real boundary is filesystem ownership.
