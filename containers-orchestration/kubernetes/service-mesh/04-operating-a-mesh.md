...
The online store now handles real customer traffic. The `web` service receives browser requests, `checkout` creates orders, `inventory` confirms stock, and `payments` authorizes cards. The familiar request path is still there: `checkout` calls `inventory` over HTTP, waits for a response, and then continues the order flow.

After the mesh rollout, that same request has more evidence and more moving parts. It leaves the `checkout` application container, passes through the `checkout` proxy, crosses the network, enters the `inventory` proxy, and then reaches the `inventory` application container. The application code still owns the business behavior, while the proxy participates in routing, connection pooling, TLS, retries, timeouts, telemetry, and policy.

**Mesh operations** means the day-to-day work of keeping that proxy-based request path reliable. For the store, operations starts with a simple habit: check the application and the proxy together. The application can be healthy while the proxy lacks CPU, carries stale configuration, starts too slowly, or returns its own network error.

This article adds one operating concern at a time. First we budget proxy CPU and memory. Then we read proxy request logs. After that we check whether proxies received the latest configuration, fix startup timing between the app and proxy, limit unnecessary proxy configuration, and roll out mesh upgrades safely. The Istio-specific names appear as we need them.
...
A **response flag** is the short field after the response code in Envoy's default access log format. In this line, the response code is `503`, and the response flag is `UF`. Many different failures can produce a `503`, and the flag gives you the proxy's reason in a form the on-call engineer can act on. The method and path show what `checkout` tried to do, the authority shows the service name it called, and the upstream host shows the Pod IP and port Envoy selected. During the online store incident, this one field decides whether the next step is checking Pod readiness, traffic policy limits, or route configuration.
...
**Proxy readiness** means the `istio-proxy` container has started, connected to `istiod`, received enough configuration, and is ready to handle captured traffic. The exact readiness behavior depends on Istio version and install settings, but the operating point stays the same: a Pod can contain a running application container while the proxy is still warming up. For the online store, `checkout` may try to call `inventory` or `payments` during boot, and the first call can fail through the proxy path before normal customer traffic even reaches the Pod.
