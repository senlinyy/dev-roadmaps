...
You manage workspaces with the `terraform workspace` subcommand.
...
Workspaces only have value if your configuration uses the workspace name to make decisions. Otherwise, every workspace creates identical resources — which does not help differentiate environments at all.
...
Here is a complete example of a workspace-based configuration that manages a web application across three environments:
...
Each apply creates a different number of instances with different types, named appropriately for the environment, stored in separate state files.
...
- [Terraform Up & Running, 3rd Edition (Yevgeniy Brikman)](https://www.terraformupandrunning.com) — Covers both workspaces and the file-layout approach, explaining the tradeoffs between them in depth.
