...
When Terraform begins any operation that might modify state — a plan, an apply, or a destroy — it first tries to acquire a lock. The mechanism depends on the backend.
...
If you are confident the original operation is dead — the process is gone, the CI job shows as failed, the machine that ran it is unreachable — force-unlock is safe. Terraform will release the lock and allow the next operation to proceed.
...
Workspaces are most useful in early-stage projects or for creating short-lived environments — spinning up a new workspace per Git branch, for example, to test a feature branch's infrastructure before merging.
...
- [Terraform Up & Running, 3rd Edition (Yevgeniy Brikman)](https://www.terraformupandrunning.com) — Chapter 3 covers state management patterns including the file-layout approach to environment isolation in depth.
