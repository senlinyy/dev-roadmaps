...
The state file is a JSON document stored by default in a file called `terraform.tfstate` in your working directory. You can open it in any text editor, though you should not edit it directly — more on that later.
...
The step where Terraform queries the cloud provider for current resource attributes is called the refresh. Before Terraform 1.0, the refresh was a separate command (`terraform refresh`). In modern Terraform, it happens automatically as part of every plan.
...
Drift happens when the real infrastructure differs from what Terraform's state file records. There are two directions this can go.
...
The state file stores everything Terraform knows about a resource after it is created. This includes:
...
Second, the sensitive flag in Terraform variable declarations (`sensitive = true`) only controls what Terraform prints in terminal output. It does not prevent the value from being written to the state file. The only way to truly protect a secret is to restrict access to the state file itself.

## The State Serial and Lineage

Inside the state file, two fields help Terraform detect conflicts and track history.

The `serial` field is a counter that increments by one every time the state is successfully modified. When you run `terraform apply` and three resources are created, the serial might go from `4` to `5`. If two people try to write the state simultaneously, the second write will see that the current state has a higher serial than the one they started from and will refuse, reporting a conflict.
