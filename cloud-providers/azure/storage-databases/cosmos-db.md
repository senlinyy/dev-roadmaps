...
Azure Cosmos DB is a fully managed, globally distributed NoSQL database service engineered for horizontal scale, low-latency point reads and writes, and high availability. NoSQL databases organize data as semi-structured documents, key-value pairs, or graph nodes rather than rigid relational tables. In Cosmos DB, data is stored as JSON-like documents within logical collections called containers. The platform automates data replication, regional failovers, and partition management, allowing applications to sustain heavy read and write throughput when the data model and partition key are designed well.
...
:::expand[Under the Hood: Logical Partitions, Physical Partitions, and RU Mathematics]{kind="design"}
Cosmos DB achieves horizontal scale by routing items through the partition key value you choose:

* **Partition Key Routing**: When an application writes a JSON document to a container, the service extracts the document's partition key value. All items with the same partition key value belong to the same logical partition, and Cosmos DB maps logical partitions onto physical partitions behind the service boundary.
* **Logical vs. Physical Partitions**: All documents sharing the same partition key value form a single logical partition. Logical partitions are mapped to physical partitions, which are the service-managed units that store data and serve request units.
    ...
    * **Partition Splits**: As your database grows, Cosmos DB can split physical partitions and move logical partitions behind the scenes. The application still addresses items by container, `id`, and partition key; the service handles the physical layout.
* **Request Unit (RU) Math**: RUs represent the normalized CPU, memory, and I/O work required to execute a database operation. A point read of a 1 KB item by `id` and partition key is the baseline 1 RU example in the official docs; writes and queries cost more depending on item size, indexing, consistency level, and query shape.

```mermaid
flowchart TD
    Doc["JSON Document (id & Partition Key)"] --> Key["Partition Key Value"]
    Key --> Route{"Service Routing"}
    Route --> P1["Physical Partition 1"]
    Route --> P2["Physical Partition 2"]
    Route --> P3["Physical Partition 3"]
...
:::expand[The Hot-Partition 429 Storm]{kind="pitfall"}
A severe NoSQL architecture failure is selecting a partition key with low cardinality (few unique values) for a high-throughput container. For example, if you partition an orders container by `orderStatus` (where values are restricted to `pending`, `shipped`, or `completed`), Cosmos DB will hash all `completed` orders to the exact same physical partition node. Under peak sales traffic, 95% of write requests target this single physical node. The physical partition's 10,000 RU throughput limit is rapidly saturated, triggering a cascade of HTTP `429 Too Many Requests` (Request Rate Too Large) throttling errors—even if the overall container-level RU budget has ample headroom.

This matches the **Amazon DynamoDB** hot partition failure mode. If you partition a DynamoDB table by a low-cardinality attribute like `IsActive`, all writes route to a single physical partition, exceeding the 1,000 WCU physical limit and throwing `ProvisionedThroughputExceededException` errors regardless of your table-level provisioned throughput.

To diagnose a hot partition in Cosmos DB:

1.  Open the Azure Portal Cosmos DB Metrics blade.
2.  Inspect the **Throughput** charts and select **Max consumed RU/second by physical partition**. A single physical partition spike indicates a hot key.
3.  Query the diagnostic logs to find the exact partition key values causing the highest request charges:
    ```sql
    SELECT c.orderStatus, COUNT(1) FROM c GROUP BY c.orderStatus
    ```

**The Fix:** Partition keys in Cosmos DB and DynamoDB are completely immutable. You cannot modify the key of an active container. To resolve this, you must provision a new container with a high-cardinality key—such as `customerId` or a synthetic compound key like `customerId_orderDate`—and execute an offline or online data migration (using the Cosmos DB Data Explorer or a bulk-executor tool) to stream data from the old container to the new one.

**Rule of thumb:** Select a partition key that distributes writes evenly across millions of unique logical values, keeping your physical partitions balanced and immune to localized 429 storms.
:::
...
![An infographic showing Cosmos DB consistency as a dial between freshness and speed](/content-assets/articles/article-cloud-providers-azure-storage-databases-cosmos-db-nosql-data-models/consistency-dial.png)

*Cosmos DB consistency is a tradeoff dial: stronger reads cost more latency, while weaker reads can be faster.*
...
![An infographic showing request unit pressure rising on a hot partition and balancing across better partition keys](/content-assets/articles/article-cloud-providers-azure-storage-databases-cosmos-db-nosql-data-models/ru-pressure.png)

*Request Units are spent per operation, and hot partitions can throttle even when the database looks mostly idle.*
...
* **Serverless**: You provision a serverless container with no predefined RU allocations. You are billed for the RUs consumed by database operations and the storage used. Serverless is useful for intermittent or unpredictable workloads, but serverless accounts run in a single Azure region and do not provide predictable throughput or latency guarantees. Throughput capacity depends on the number of physical partitions, so treat it as a burst-friendly model rather than a replacement for provisioned RU/s on critical steady workloads.
...
Cosmos DB includes a built-in Time-to-Live (TTL) mechanism that automates document deletion after an item reaches its configured expiry. This is highly effective for temporary data assets like idempotency records, session tokens, and telemetry logs.

You can configure TTL behavior at two distinct layers:

* **Container-Level Default**: You set a default TTL in seconds (e.g., `86400` for 24 hours) on the container. Every document written to the container becomes eligible for automatic deletion after that interval from its last update.
...
* **Choose Cosmos DB when**: Your data is document-shaped, queries are known in advance, you need low-latency point reads and writes, and you need horizontal scaling or global multi-region replication in a provisioned-throughput account.
...
![An infographic showing Cosmos DB items routed by partition key into balanced physical partitions, with hot partition risk, RU budget, consistency, TTL, and access pattern controls](/content-assets/articles/article-cloud-providers-azure-storage-databases-cosmos-db-nosql-data-models/cosmos-db-partition-map.png)

*Use this as the Cosmos DB partition map: the partition key routes every item, balanced distribution keeps RU usage healthy, and hot partitions reveal access patterns that need redesign.*
...
* [Understanding request unit consumption](https://learn.microsoft.com/en-us/azure/cosmos-db/understand-request-unit-consumption)
