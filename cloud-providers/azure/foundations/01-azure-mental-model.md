...
4.  **Resource Groups**: A mandatory logical folder inside a subscription. Every single Azure resource you create must live inside exactly one resource group. Unlike folders on your computer's filesystem, resource groups cannot be nested—you cannot put a resource group inside another resource group. These groups are designed to hold resources that share a lifecycle, allowing you to deploy, update, or delete a collection of related services as a single unit.
...
### 1. Token Validation
ARM decrypts the JSON Web Token (JWT) passed in your CLI request header. It contacts Microsoft Entra ID to prove your caller identity is active, the cryptographic signature is valid, and the token has not expired. If your session is stale, the request is rejected immediately at the front gate with a `401 Unauthorized` status.
