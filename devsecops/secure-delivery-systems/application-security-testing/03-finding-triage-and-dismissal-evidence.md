...
description: "Turn noisy scanner findings into clear decisions with source, severity, exploitability, reachability, ownership, due dates, dismissal evidence, and verification."
overview: "Start with a noisy alert board for the checkout API team. Then learn how to treat each finding as a lead, check the source and evidence, compare severity with exploitability and reachability, assign owners and due dates, separate false positives from accepted risk, write useful dismissal notes, and verify fixes before closing alerts."
...
The checkout API team opens its security board on Monday morning. CodeQL has three new alerts. Secret scanning shows one blocked payment-provider token. ZAP has a staging report with twelve warnings. A targeted authorization test failed once overnight. The board is full of red and yellow labels, and every row looks urgent at first glance.

A **finding** is a lead from a tool, test, reviewer, or report. It says, "Here is something worth checking." It still needs product context before it turns into a final decision. For `checkout-api`, a CodeQL alert may point to user input reaching a database query. The team still has to ask whether that code path runs, whether the input is controlled, whether the route is deployed, and who owns the fix.

The team now has several security signals. **CodeQL** is GitHub's code-scanning engine for finding risky code paths, and it comments on pull requests when it sees a data-flow issue. **Secret scanning** looks for credentials such as API keys in commits and either blocks the push or raises an alert. **ZAP**, the OWASP Zed Attack Proxy, sends HTTP requests to the staging API and reports risky responses. Targeted tests check important authorization rules. This is a good place to be, and it creates a new job: someone has to decide what each finding means.
...
When triage is missing, security tools create a messy queue. Developers see hundreds of alerts and lose trust. Security reviewers chase old warnings that no longer match the product. Real issues hide beside noisy ones. Managers ask whether the product is safe, and the team can only point to a long list of unowned findings.
...
| Is the affected route deployed? | Unused code has different urgency from a live route |
...
Severity is useful, and the checkout team still needs **exploitability** and **reachability** before choosing an action.
...
Many teams use **SSVC**, Stakeholder-Specific Vulnerability Categorization, to make this more decision-oriented. SSVC uses decision points such as exploitation status, technical impact, mission impact, and exposure to choose an action. The useful idea for beginners is this: vulnerability priority should include product context alongside a generic score.
...
These targets are examples for the checkout team. A bank, hospital system, public-sector service, or consumer checkout platform may need stricter timelines. The important part is that the team writes the expectation down before a wave of alerts arrives.

Some findings will close without a code fix. The next section separates those closure reasons so the board stays honest.
...
A **false positive** means the scanner reported a weakness that product evidence shows is safe. Maybe CodeQL missed the team's custom sanitizer. Maybe a fake secret in a test fixture matched a provider pattern, but the provider rejects it. A false-positive dismissal should explain the proof, because future readers need to know why the alert was safe to close.
...
False positive should stay reserved for alerts where the scanner truly got the code wrong. If the finding is real with lower urgency, call it real and assign the right due date. If the team accepts the risk, say accepted risk. If the finding is noisy because of test fixtures, mark it test-only and make the fixture obviously fake.

During audits and incidents, vague dismissals create pain. Clear dismissals tell the story.
...
```markdown
False positive. CodeQL misses sanitizeOrderSearchTerm in src/security/search.ts.
...
The format can be simple. Every dismissal should answer: why is closing this acceptable, who can verify the reasoning, and when should the team revisit it?
...
Fixing code and closing a finding are related steps. They should happen together, and the team should still collect proof. A developer can change a line and still leave the route vulnerable. A scanner can go green because it lost coverage. Verification closes that gap.
...
Every weekday, a rotating engineer checks new security alerts for the checkout repository. This small rotation belongs to normal engineering work. The engineer groups obvious duplicates, labels each new finding by source, and assigns an initial owner.
...
For high-risk findings, the team responds before the weekly meeting. A reachable production auth bypass, leaked live credential, confirmed injection, or cross-user data exposure gets immediate attention. The release pauses until the team fixes or mitigates the issue.

For normal findings, the team runs a weekly 30-minute review. They check new alerts, old findings near due dates, accepted risks near expiration, and noisy rules that need tuning. The meeting output is a small set of decisions instead of a long discussion about every scanner row.
...
`Summary` names the issue in plain language. `Source` says which tool, test, or person raised it. `Affected component` points to the service, route, package, or secret. `Reachability` records whether an attacker can hit the path in this product. `Impact` explains what could happen. `Decision`, `Owner`, and `Due date` turn the finding into work. `Evidence` records the proof behind the decision. `Verification plan` says how the team will know the fix or dismissal is safe.
...
- [CISA Stakeholder-Specific Vulnerability Categorization](https://www.cisa.gov/stakeholder-specific-vulnerability-categorization-ssvc) - Describes SSVC decision-tree prioritization for vulnerability response.
