...
description: "Test a running application with DAST, OpenAPI-driven API scans, authenticated flows, active checks, passive checks, and safe staging gates."
overview: "Start with the checkout API running in staging, where routing, authentication, configuration, and real HTTP responses now exist. Then follow the team as they add OWASP ZAP, OpenAPI-driven API scanning, authenticated test users, safe active checks, CI reports, and a clean handoff into finding triage."
...
The staging deployment finishes, and `checkout-api` has a real URL: `https://checkout-staging.example.com`. The gateway routes requests to the service. Authentication tokens work. CORS rules come from the deployed config. Error handlers return real HTTP responses. Fake payment and email services stand behind the app so the team can test without charging cards or sending receipts.

The previous article gave the team early feedback in pull requests. CodeQL caught risky SQL construction. Secret scanning and push protection caught exposed credentials. Those checks still help, but this staging deployment has behavior that only appears once the app is running.

A running application has headers, cookies, redirects, CORS rules, authentication tokens, rate limits, error responses, deployed configuration, reverse proxies, and real authorization checks. A scanner that only reads code may miss a staging setting that exposes stack traces. It may struggle to prove whether customer A can read customer B's order. It may miss a route that exists because a gateway maps traffic in front of the service.
...
OWASP's API Security Top 10 2023 puts **Broken Object Level Authorization** at API1. That issue appears when an API lets a caller access someone else's object by changing an ID. In the checkout API, a customer might call:
...
For API scanners, route discovery is the first practical problem. A web crawler can follow links on a website. APIs often have no links to follow because clients already know which endpoints to call. An **OpenAPI definition** gives the scanner that missing map. OpenAPI is a machine-readable description of API paths, methods, parameters, request bodies, response bodies, and authentication schemes.
...
In the small example above, `openapi: 3.0.3` names the OpenAPI version. `paths` lists the route the scanner can call. `parameters` tells the scanner that `orderId` belongs in the URL path. `security` says the route needs a bearer token. `components.securitySchemes` defines what that bearer token means for the API.
...
Many useful API routes require authentication. **Authentication** proves who the caller is, usually through a session cookie, API key, or bearer token. An unauthenticated scanner sees only public routes such as `/health`, `/login`, and `/docs`. That misses most checkout behavior.
...
For the checkout API, an active scan should run against staging. Production stays outside the active scan scope because the scanner sends unusual input on purpose. The staging environment should use fake payment credentials, fake email delivery, fake customer records, and resettable databases. The scanner should never charge a real card, send a real receipt, or create a real shipment.
...
The plan has three important pieces. `contexts.urls` defines the allowed target scope. The `openapi` job imports the route map from staging and sets the base URL for requests. The `activeScan` job tells ZAP to send attack-style payloads inside that context. The `report` job writes machine-readable evidence that CI can upload and triage can inspect.
...
The command uses `TARGET` and `OPENAPI` so the scan target stays visible. `TOKEN` comes from a staging-only test endpoint and travels into ZAP through the replacer configuration as the `Authorization` header. `-J zap-report.json` writes structured output for automation, `-r zap-report.html` writes a human report, and the final `jq` command extracts the fields a triager usually needs first: risk, rule name, and URL.
...
```ts
it("blocks one customer from reading another customer's order", async () => {
...
Every night, the team runs a deeper scan. The nightly scan can use more active rules, more test users, and a reset staging database. It can take longer because it runs outside the developer's pull request path.
...
The final handoff goes to triage because scanners produce findings, and the team turns those findings into decisions.
...
- [OWASP ZAP: Authentication](https://www.zaproxy.org/docs/authentication/) - Explains how ZAP models authenticated application scans.
...
- [OWASP Top Ten Project](https://owasp.org/www-project-top-ten/) - Provides broad web application risk categories.
