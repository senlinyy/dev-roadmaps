...
description: Use admission checks to reject risky Kubernetes objects before they are stored and scheduled.
overview: Admission control is the last review step before a Kubernetes object enters the cluster. You will use built-in admission, validation policy, and webhook thinking to protect devpolaris-orders releases.
...
Admission control is the review stage after authentication and authorization but before an object is stored. It exists so the cluster can reject risky manifests even when the caller has permission to create them. The devpolaris-orders release should fail before scheduling if it tries to run privileged pods, use mutable images, or miss required labels.
...
metadata:
  name: require-orders-image-digest
spec:
  matchConstraints:
    resourceRules:
      - apiGroups: ["apps"]
  ...
  validations:
    - expression: "object.metadata.namespace != 'devpolaris-orders' || object.spec.template.spec.containers.all(c, c.image.contains('@sha256:'))"
      message: "devpolaris-orders deployments must pin container images by digest"
```
...
```bash
$ kubectl apply --dry-run=server -f deployment-with-tag.yaml
Error from server (Forbidden): admission webhook or validating admission policy denied the request:
devpolaris-orders deployments must pin container images by digest

$ kubectl apply --dry-run=server -f deployment-with-digest.yaml
...
```text
2026-05-08T19:44:02Z release=orders-api
admission denied: devpolaris-orders deployments must pin container images by digest
submitted image: ghcr.io/devpolaris/orders-api:latest
fix direction: deploy ghcr.io/devpolaris/orders-api@sha256:<digest> from the signed build record
```
...
Use this worksheet when a pull request or incident touches admission control for devpolaris-orders. It keeps the review tied to evidence from the cluster instead of opinions about whether a setting looks strict enough.

| Check | Command or evidence | Expected result |
|-------|---------------------|-----------------|
| Server dry run rejects mutable tags | `kubectl apply --dry-run=server -f deployment-with-tag.yaml` | denied |
| Server dry run accepts digest image | `kubectl apply --dry-run=server -f deployment-with-digest.yaml` | configured server dry run |
| Policy targets Deployments only | `kubectl get validatingadmissionpolicy require-orders-image-digest -o yaml` | resources deployments |
| Message tells the developer what to fix | `kubectl apply --dry-run=server -f deployment-with-tag.yaml` | must pin container images by digest |
| Break-glass path is owned | `kubectl get validatingadmissionpolicybinding -o yaml` | documented exception owner |
...
An admission policy with a vague denial message creates support work. Keep the message concrete enough that a developer knows which field to change and where to find the expected value.
...
Use this drill to turn admission control from a file review into an operating habit. The scenario starts with server dry run denial in the devpolaris-orders namespace. The goal is to collect enough evidence to decide whether to fix the workload, fix the policy, or open a short exception.

1. Name the affected object: devpolaris-orders Deployment.
2. Name the protecting artifact: ValidatingAdmissionPolicy and binding.
...
Namespace: devpolaris-orders
Symptom: server dry run denial
Primary object: devpolaris-orders Deployment
Security artifact: ValidatingAdmissionPolicy and binding
...
| Policy object | Names the Kubernetes object that made or should make the decision | ValidatingAdmissionPolicy and binding |
...
- [Kubernetes Admission Controllers](https://kubernetes.io/docs/reference/access-authn-authz/admission-controllers/) - Use this as the canonical reference for the Kubernetes behavior described in this article.
- [Dynamic Admission Control](https://kubernetes.io/docs/reference/access-authn-authz/extensible-admission-controllers/) - Use this as the canonical reference for the Kubernetes behavior described in this article.
- [Validating Admission Policy](https://kubernetes.io/docs/reference/access-authn-authz/validating-admission-policy/) - Use this as the canonical reference for the Kubernetes behavior described in this article.
