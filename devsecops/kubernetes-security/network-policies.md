...
description: Use Kubernetes NetworkPolicy objects to limit which pods can talk to each other and to required outside services.
overview: Network policies turn pod traffic into an explicit allow list. You will protect devpolaris-orders by allowing ingress from the web tier and egress only to the database, DNS, and required APIs.
...
NetworkPolicy is Kubernetes traffic policy for pods. It decides which connections are allowed based on pod selectors, namespace selectors, ports, and direction. The devpolaris-orders API should accept traffic from the web tier and send traffic only to DNS, the orders database, and required payment APIs.
...
kind: NetworkPolicy
metadata:
  name: orders-api-allow-web-and-db
  namespace: devpolaris-orders
spec:
  podSelector:
    matchLabels:
...
```bash
$ kubectl get networkpolicy -n devpolaris-orders
NAME                         POD-SELECTOR                     AGE
orders-api-allow-web-and-db   app=devpolaris-orders-api        12m

$ kubectl describe networkpolicy orders-api-allow-web-and-db -n devpolaris-orders
PodSelector: app=devpolaris-orders-api
Allowing ingress traffic: TCP 8080 from namespace devpolaris-web and pods tier=web
Allowing egress traffic: TCP 5432 to pods app=orders-postgres, UDP 53 to kube-system
```
...
```text
2026-05-08T19:22:08Z service=orders-api error="dial tcp 10.42.16.20:5432: i/o timeout"

Debug notes:
selected pod labels: app=devpolaris-orders-api,tier=api
database pod labels: app=orders-db
policy allows: app=orders-postgres
fix direction: align the egress podSelector with the real database label or fix the database label
```
...
Use this worksheet when a pull request or incident touches NetworkPolicy for devpolaris-orders. It keeps the review tied to evidence from the cluster instead of opinions about whether a setting looks strict enough.

| Check | Command or evidence | Expected result |
|-------|---------------------|-----------------|
| Policy selects the API pods | `kubectl get netpol orders-api-allow-web-and-db -n devpolaris-orders -o jsonpath={.spec.podSelector.matchLabels.app}` | devpolaris-orders-api |
| Ingress has an allowed web source | `kubectl describe netpol orders-api-allow-web-and-db -n devpolaris-orders` | namespace devpolaris-web and pods tier=web |
| Database egress label matches reality | `kubectl get pod -n devpolaris-orders -l app=orders-postgres --show-labels` | at least one pod |
| DNS egress exists | `kubectl describe netpol orders-api-allow-web-and-db -n devpolaris-orders` | UDP 53 to kube-system |
| Unrelated pod cannot call the API | `kubectl exec test-client -n devpolaris-tools -- curl -m 3 devpolaris-orders-api.devpolaris-orders:8080/health` | timeout or connection denied |
...
When a policy appears to do nothing, first confirm that your cluster network plugin enforces NetworkPolicy and that the policy selects at least one pod. A valid object with a selector that matches zero pods protects nothing.
...
Use this drill to turn NetworkPolicy from a file review into an operating habit. The scenario starts with database timeout after policy merge in the devpolaris-orders namespace. The goal is to collect enough evidence to decide whether to fix the workload, fix the policy, or open a short exception.

1. Name the affected object: devpolaris-orders-api pod labels.
2. Name the protecting artifact: NetworkPolicy selectors and ports.
...
Namespace: devpolaris-orders
Symptom: database timeout after policy merge
Primary object: devpolaris-orders-api pod labels
Security artifact: NetworkPolicy selectors and ports
...
| Policy object | Names the Kubernetes object that made or should make the decision | NetworkPolicy selectors and ports |
...
- [Kubernetes Network Policies](https://kubernetes.io/docs/concepts/services-networking/network-policies/) - Use this as the canonical reference for the Kubernetes behavior described in this article.
- [Kubernetes Service Concepts](https://kubernetes.io/docs/concepts/services-networking/service/) - Use this as the canonical reference for the Kubernetes behavior described in this article.
- [Kubernetes DNS for Services and Pods](https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/) - Use this as the canonical reference for the Kubernetes behavior described in this article.
