...
description: Store, mount, rotate, and audit Kubernetes Secrets without treating base64 as real protection.
overview: Kubernetes Secrets are API objects for sensitive values, not a complete secret-management system by themselves. You will trace how devpolaris-orders receives database credentials and how teams reduce exposure.
...
A Kubernetes Secret is an API object for sensitive configuration such as passwords, tokens, and certificates. It is not encryption by itself, and base64 is only encoding. The devpolaris-orders API receives a database password through a Secret, while the platform team controls storage, RBAC, mounting, and rotation.
...
stringData:
  username: orders_app
  password: replace-me-through-secret-manager
---
apiVersion: apps/v1
kind: Deployment
metadata:
...
```bash
$ kubectl get secret orders-db-credentials -n devpolaris-orders
NAME                    TYPE     DATA   AGE
orders-db-credentials   Opaque   2      4m
...
```text
2026-05-08T19:31:18Z level=error service=orders-api
missing required environment variable ORDERS_DB_PASSWORD

kubectl describe pod shows:
Warning  Failed  Error: secret "orders-db-credentials" not found
fix direction: create the Secret in devpolaris-orders or correct the secretKeyRef name
```
...
Use this worksheet when a pull request or incident touches Secrets for devpolaris-orders. It keeps the review tied to evidence from the cluster instead of opinions about whether a setting looks strict enough.

| Check | Command or evidence | Expected result |
|-------|---------------------|-----------------|
| Secret exists in the same namespace | `kubectl get secret orders-db-credentials -n devpolaris-orders` | DATA 2 |
| Release identity cannot read it | `kubectl auth can-i get secret/orders-db-credentials --as=system:serviceaccount:devpolaris-orders:orders-release -n devpolaris-orders` | no |
| Pod references the expected keys | `kubectl get deploy devpolaris-orders-api -n devpolaris-orders -o yaml` | username and password secretKeyRef |
| Secret is not printed in logs | `kubectl logs deploy/devpolaris-orders-api -n devpolaris-orders | grep ORDERS_DB_PASSWORD` | no output |
| Rotation has a restart plan | `kubectl rollout status deploy/devpolaris-orders-api -n devpolaris-orders` | deployment successfully rolled out |
...
A base64 value in a Secret is not protected from anyone who can read the Secret. The fix is RBAC, encryption at rest, careful mounting, external rotation, and log hygiene, not a different base64 command.
...
Use this drill to turn Secrets from a file review into an operating habit. The scenario starts with missing database password during rollout in the devpolaris-orders namespace. The goal is to collect enough evidence to decide whether to fix the workload, fix the policy, or open a short exception.

1. Name the affected object: orders-db-credentials Secret.
2. Name the protecting artifact: secretKeyRef, RBAC, and rotation record.
...
Namespace: devpolaris-orders
Symptom: missing database password during rollout
Primary object: orders-db-credentials Secret
Security artifact: secretKeyRef, RBAC, and rotation record
...
| Policy object | Names the Kubernetes object that made or should make the decision | secretKeyRef, RBAC, and rotation record |
...
- [Kubernetes Secrets](https://kubernetes.io/docs/concepts/configuration/secret/) - Use this as the canonical reference for the Kubernetes behavior described in this article.
- [Encrypt Secret Data at Rest](https://kubernetes.io/docs/tasks/administer-cluster/encrypt-data/) - Use this as the canonical reference for the Kubernetes behavior described in this article.
- [Good Practices for Kubernetes Secrets](https://kubernetes.io/docs/concepts/security/secrets-good-practices/) - Use this as the canonical reference for the Kubernetes behavior described in this article.
