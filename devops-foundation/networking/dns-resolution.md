...
DNS exists because the obvious alternative does not scale. In the early ARPANET, every machine kept a single `HOSTS.TXT` file listing every other host on the network by name. SRI International maintained the master copy and admins downloaded it by FTP whenever it changed. That worked when there were a few hundred hosts; by the early 1980s the file was megabytes long, hours out of date, and a single point of failure for the whole network. DNS replaced it with a hierarchy that delegates: the root knows who runs `.com`, `.com` knows who runs `example.com`, and `example.com` knows its own records. No one has to download the whole internet to look up one name. Your `/etc/hosts` file on a modern machine is the last vestige of HOSTS.TXT, useful only for a handful of local overrides.
...
TTL is set per record on purpose, because different records have different update patterns. Your apex `A` record might change once a year and benefits from a long TTL of 3600 or 86400 seconds: every cache hit is one fewer query the authoritative server has to answer, and `.com`'s authoritative servers handle hundreds of billions of queries a day. A blue/green deployment record that you flip during cutovers wants a TTL of 60 seconds so failover is fast. Per-record TTL lets you make that trade-off independently for each name instead of choosing a single number for the whole zone. The cost of a short TTL is more load on the authoritative servers; the cost of a long TTL is slower failover. You decide which one to pay for each record.
