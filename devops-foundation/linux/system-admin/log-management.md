...
The newer system is **systemd-journald**. It arrived with systemd in 2010 and works differently: it captures the stdout and stderr of every service systemd starts (the same two output streams that `console.log` and `console.error` write to in Node, or that `print()` and `sys.stderr.write()` use in Python), and stores entries in a structured binary format under `/var/log/journal/`. Each entry is indexed by metadata fields like the service name (`_SYSTEMD_UNIT`), process ID (`_PID`), user ID (`_UID`), and priority. Because the files are binary, you cannot `grep` them directly. You query them through a dedicated tool called `journalctl`.
