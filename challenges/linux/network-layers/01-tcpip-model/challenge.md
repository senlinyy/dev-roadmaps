...
A new on-call engineer needs a quick map of which services on `polaris-edge-01` use TCP vs UDP and what application-layer protocols they speak. The platform team exported the listening services from `ss -tlnp` / `ss -ulnp` into a flat inventory at `/var/log/network/listeners.snapshot`, one service per line in the format `name  proto/port  app-protocol`.

You start in `/home/dev`. Your job:

1. **Inspect the service inventory** at `/var/log/network/listeners.snapshot` so you understand the columns and the services in scope.
