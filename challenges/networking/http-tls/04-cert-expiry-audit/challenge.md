...
4. **Write `/home/dev/reports/api-cert-renewal.note`** with the leaf expiry, CA expiry, and SANs that need coverage.
5. **Print the renewal note** so the comparison is visible in the terminal history.

The grader requires you to use `cat`, `grep`, and `echo`, and checks that your note records the leaf expiry `Mar  5 09:14:01 2025 GMT`, the CA expiry `Jan  1 00:00:00 2033 GMT`, and the SAN `DNS:api.example.com`.
